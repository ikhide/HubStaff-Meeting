...
async function processMeetings(domain, hubId, q) {
  try {
    ...
    const lastPulledDate = new Date("2001-01-01");
    // const lastPulledDate = new Date(
    //   account.lastPulledDates.meetings || "2020-01-01"
    // );
    const now = new Date();
    ...
    while (hasMore) {
      ...
      const searchResult = await retryOperation(async () => {
        ...
        console.log(`Fetched ${result?.results?.length || 0} meetings`);
        return result;
      });
      ...
      // Process meetings in chunks to avoid memory and rate limitation issues
      const chunks = _.chunk(searchResult.results, 20);
      for (const chunk of chunks) {
        await Promise.all(
          chunk.map(async (meeting) => {
            const attendees = await getMeetingAttendees(
              hubspotClient,
              meeting.id,
              contactEmailCache
            );
            ...
            if (attendees.length) {
              await processMeetingAction(meeting, attendees, q);
              processed++;
            }
          })
        );
      ...
      console.log(`Processed ${processed} meetings so far`);
    ...
    console.log(`Meeting processing complete. Total processed: ${processed}`);
...
async function getMeetingAttendees(hubspotClient, meetingId, emailCache) {
  if (!meetingId) {
  ...
  try {
    ...
    if (!contactIds.length) {
      console.log(`No contacts found for meeting ${meetingId}`);
      return [];
    }

    // Process uncached contacts in batches
    const uncachedIds = contactIds.filter((id) => !emailCache.has(id));
    if (uncachedIds.length > 0) {
      const batches = _.chunk(uncachedIds, 100);
        ...
        if (batchResponse?.results) {
          batchResponse.results.forEach((contact) => {
            if (contact?.properties?.email) {
              emailCache.set(contact.id, contact.properties.email);
            }
          });
        }
      }
    ...
    // Get cached emails
    const attendeeEmails = contactIds
      .map((id) => emailCache.get(id.toString()))
      .filter(Boolean);
...
async function processMeetingAction(meeting, attendees, q) {
  const isNew =
